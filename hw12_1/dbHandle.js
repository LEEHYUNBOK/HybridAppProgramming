  ...
  db.transaction(function(tr){
 	var selectSQL = 'select * from book';
  	tr.executeSql(selectSQL, [], function(tr, rs){
       ...
       $('#author4').val(rs.rows.item(index).author);
		...
		var updateSQL = 'update book set type = ?, name = ?, author = ? where name = ?';
     	tr.executeSql(updateSQL, [type, new_name, author, old_name], function(tr, rs){
	         ...
	         alert('도서명 ' + $('#sBookName2').val() + ' 이 수정되었습니다');
	   		 $('#bookName2').val(''); $('#sBookName2').val('');$('#author2').val('');
   ...
   db.transaction(function(tr){
	  ...
	  tr.executeSql(deleteSQL, [name], function(tr, rs){
	     ...
	     alert('도서명 ' + $('#bookName3').val() + ' 이 삭제되었습니다');
	   	 $('#bookType3').val(''); $('#bookName3').val(''); $('#sBookName3').val('');$('#author2').val('');
		}, function(tr, err){
				alert('DB오류 ' + err.message + err.code);
			}
		);
...
function selectBook2(name){
   db.transaction(function(tr){
	 var selectSQL = 'select type, name, author from book where name=?';
       ...
       $('#author2').val(rs.rows.item(0).author);
	 	});
   });
}
...
function selectBook3(name){
   db.transaction(function(tr){
 	 var selectSQL = 'select type, name, author from book where name=?';
       ...
       $('#author3').val(rs.rows.item(0).author);
		}, function(tr, err){
				alert('DB오류 ' + err.message + err.code);
			}
		);
   ...
   db.transaction(function(tr){
 	 var selectSQL = 'select type, name, author from book where name=?';
  	 tr.executeSql(selectSQL, [name], function(tr, rs){
         ...
         $('#author4').val(rs.rows.item(0).author);
		}, function(tr, err){
				alert('DB오류 ' + err.message + err.code);
			}
		);
