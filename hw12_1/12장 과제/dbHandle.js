...
// 데이터베이스 생성 및 오픈
function openDB(){
   db = window.openDatabase('bookDataBase', '1.0', '북DB', 1024*1024); 
   console.log('1_DB 생성...'); 
} 
// 테이블 생성 트랜잭션 실행
    ...
    db.transaction(function(tr){
      ...
      var updateSQL = 'update book set type = ?, name = ? , author = ? where name = ?';          
        tr.executeSql(updateSQL, [type, new_name, author, old_name], function(tr, rs){    
          ...
          $('#bookAuthor2').val(''); $('#sBookAuthor2').val('');
      }, function(tr, err){
            alert('DB오류 ' + err.message + err.code);
         }
      );
   ...
   db.transaction(function(tr){
     ...
     tr.executeSql(deleteSQL, [name], function(tr, rs){    
        ...
        alert('도서명 ' + $('#bookName3').val() + ' 이 삭제되었습니다');           
          $('#bookType3').val(''); $('#bookName3').val(''); $('#sBookName3').val('');$('#bookAuthor3').val('');           
      }, function(tr, err){
            alert('DB오류 ' + err.message + err.code);
         }
      );
